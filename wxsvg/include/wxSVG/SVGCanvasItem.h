...
struct wxSVGMark {
  ...
  wxSVGMark(double aX, double aY, double aAngle, Type aType)
...
public:
  ...
  virtual wxSVGRect GetResultBBox(const wxCSSStyleDeclaration& style,
                                  const wxSVGMatrix* matrix = NULL) {
    return GetBBox(matrix);
  }
...
public:
  ...
  void CurveToCubic(double x1, double y1, double x2, double y2, double x,
                    double y, bool relative = false);
  void CurveToCubicSmooth(double x2, double y2, double x, double y,
                          bool relative = false);
  void CurveToQuadratic(double x1, double y1, double x, double y,
                        bool relative = false);
...
public:
  ...
  virtual void Init(wxSVGTextElement& element,
...
protected:
  ...
  int m_textAnchorBeginIndex; /** index of first chunk with current text anchor
                               */
  double
      m_textAnchorBeginPos; /** x-coordinate of text with current text anchor */
  wxCSS_VALUE m_dominantBaseline;   /** current dominant baseline */
  int m_dominantBaselineBeginIndex; /** index of first chunk with current
                                       baseline */
  virtual void Init(wxSVGTSpanElement& element,
                    const wxCSSStyleDeclaration& style, wxSVGMatrix* matrix);
  virtual void InitChildren(wxSVGTextPositioningElement& element,
                            const wxCSSStyleDeclaration& style,
                            wxSVGMatrix* matrix);
  virtual void AddChunk(const wxString& text,
                        const wxCSSStyleDeclaration& style,
                        wxSVGMatrix* matrix);
  ...
  virtual void InitText(const wxString& text,
                        const wxCSSStyleDeclaration& style,
                        wxSVGMatrix* matrix) = 0;
};
...
public:
  wxSVGCanvasImage()
      : wxSVGCanvasItem(wxSVG_CANVAS_ITEM_IMAGE),
        m_x(0),
        m_y(0),
        m_width(0),
        m_height(0),
        m_defHeightScale(1),
        m_svgImageData(NULL) {}
  wxSVGCanvasImage(wxSVGCanvasItemType type)
      : wxSVGCanvasItem(type),
        m_x(0),
        m_y(0),
        m_width(0),
        m_height(0),
  ...
  const wxSVGPreserveAspectRatio& GetPreserveAspectRatio() {
