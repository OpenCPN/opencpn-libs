...
public:
  wxSvgXmlAttribute() {}
  wxSvgXmlAttribute(const wxString& name, const wxString& value)
...
public:
  ...
  void Add(const wxSvgXmlAttrHash& value) {
    ...
    for (it = value.begin(); it != value.end(); ++it) push_back(*it);
  }
};
...
// element with name="title", irrelevant content and one child
// (wxSVGXML_TEXT_NODE with content="hi").
//
...
public:
  ...
  virtual wxSvgXmlNode* CloneNode(bool deep = true) {
    return new wxSvgXmlNode(*this);
  }
  ...
  inline wxSvgXmlNode* InsertBefore(wxSvgXmlNode* newChild,
                                    wxSvgXmlNode* refChild) {
    InsertChild(newChild, refChild);
    return newChild;
  }
...
public:  // W3C DOM Methods
  ...
  inline wxSvgXmlNode* AppendChild(wxSvgXmlNode* child) {
    if (!m_root) SetRoot(child);
    return child;
  }
  inline wxSvgXmlNode* RemoveChild(wxSvgXmlNode* child) {
    if (m_root != child) return NULL;
    m_root = NULL;
    return child;
  }
