 ...
 * callback. See \ref libusb_hotplug_event \param[in] flags bitwise or of
 * hotplug flags that affect registration. See \ref libusb_hotplug_flag
 * \param[in] vendor_id the vendor id to match or \ref LIBUSB_HOTPLUG_MATCH_ANY
 * \param[in] product_id the product id to match or \ref
 * LIBUSB_HOTPLUG_MATCH_ANY \param[in] dev_class the device class to match or
 * \ref LIBUSB_HOTPLUG_MATCH_ANY \param[in] cb_fn the function to be invoked on
 * a matching event/device \param[in] user_data user data to pass to the
 * callback function \param[out] callback_handle pointer to store the handle of
 * the allocated callback (can be NULL) \returns LIBUSB_SUCCESS on success
...
enum libusb_option {
   ...
   *
   * This option should be set immediately after calling libusb_init(),
   ...
   * is limited.
   *
   * For LIBUSB_API_VERSION 0x01000108 it was called
