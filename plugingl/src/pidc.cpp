...
void piDC::DrawGLThickLines( int n, wxPoint points[],wxCoord xoffset, wxCoord yoffset, wxPen pen, bool b_hiqual )
{
#ifdef ocpnUSE_GL
    if(n < 2)
        return;
...
void piDC::DrawPolygon( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset, float scale, float angle )
{
    if( dc )
        ...
        if(n > 4){
            ...
            glUniform4fv(bcolloc, 1, bcolorv);
            if(bcolloc == -1)
                wxLogMessage(_("piDC::DrawPolygon: bcolloc -1"));
        ...
        if(n > 3) {
           if(ConfigureBrush()) {       // Check for transparent brush
            ...
            if(n == 3){
                ...
                checkGlError("glEnableVertexAttribArray( mUvAttrib )", "piDC", __LINE__);
...
void APIENTRY piDCbeginCallback( GLenum type )
{
    glBegin( type );
}

void APIENTRY piDCendCallback()
{
    glEnd();
}
#endif
...
void __CALL_CONVENTION piDCPatternbeginCallback(GLenum type)
{
    glBegin( type );
}

void __CALL_CONVENTION piDCPatternendCallback()
{
    glEnd();
}
#endif
...
//        glUniform1f(yo, fmod(points[0].y, textureSize.y));
        ...
        gluTessCallback( m_tobj, GLU_TESS_BEGIN, (_GLUfuncptr) &piDCPatternbeginCallback );
        gluTessCallback( m_tobj, GLU_TESS_END, (_GLUfuncptr) &piDCPatternendCallback );
        gluTessCallback( m_tobj, GLU_TESS_COMBINE, (_GLUfuncptr) &piDCPatterncombineCallback );
        gluTessCallback( m_tobj, GLU_TESS_ERROR, (_GLUfuncptr) &piDCPatternerrorCallback );
...
void piDC::DrawPolygonsTessellated( int n, int npoints[], wxPoint points[], wxCoord xoffset, wxCoord yoffset )
{
#if 1
    if( dc ) {
        int prev = 0;
...
void piDC::DrawPolygons(int n, int npoint[], wxPoint points[], wxCoord xoffset, wxCoord yoffset)
{
    if(g_textureId >= 0)
...
void piDC::DrawTexture( wxRect texRect, int width, int height, float scaleFactor, wxPoint position, float rotation, wxPoint rPivot)
{
        float w = width;
        float h = height;
...
void piDC::DrawTextureAlpha( wxRect texRect, int width, int height, float scaleFactor, wxPoint position, float rotation, wxPoint rPivot)
{
        float w = width;
        float h = height;
...
void piDC::RenderSingleTexture(float *coords, float *uvCoords, PlugIn_ViewPort *vp, float dx, float dy, float angle_rad) {
#ifdef USE_ANDROID_GLES2
...
int piDC::ArcSectorPoints( wxPoint *&points, wxCoord xc, wxCoord yc, wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2, wxCoord x3, wxCoord y3, wxCoord x4, wxCoord y4, bool bHighQuality )
{
    double y1yc, x1xc, y4yc, x4xc;
    y1yc = y1-yc;
    x1xc = x1-xc;
