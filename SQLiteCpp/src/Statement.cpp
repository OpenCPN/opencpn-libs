...
// Compile and register the SQL query for the provided SQLite Database
// Connection
Statement::Statement(Database& aDatabase, const char* apQuery)
    : mQuery(apQuery),
      mStmtPtr(aDatabase.mpSQLite,
               mQuery),  // prepare the SQL query, and ref count (needs Database
                         // friendship)
      mColumnCount(0),
      mbHasRow(false),
      mbDone(false) {
...
// Clears away all the bindings of a prepared statement (can be associated with
// #reset() above).
...
// Bind an int value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in the
...
// Bind a 32bits unsigned int value to a parameter "?", "?NNN", ":VVV", "@VVV"
// or "$VVV" in the SQL prepared statement
void Statement::bind(const int aIndex, const unsigned aValue) {
  const int ret = sqlite3_bind_int64(mStmtPtr, aIndex, aValue);
  check(ret);
}

// Bind a 64bits int value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV"
...
// Bind a double (64bits float) value to a parameter "?", "?NNN", ":VVV", "@VVV"
// or "$VVV" in the SQL prepared statement
...
// Bind a string value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in
// the SQL prepared statement
void Statement::bind(const int aIndex, const std::string& aValue) {
  const int ret =
      sqlite3_bind_text(mStmtPtr, aIndex, aValue.c_str(),
                        static_cast<int>(aValue.size()), SQLITE_TRANSIENT);
  check(ret);
}

// Bind a text value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in the
// SQL prepared statement
void Statement::bind(const int aIndex, const char* apValue) {
  const int ret =
      sqlite3_bind_text(mStmtPtr, aIndex, apValue, -1, SQLITE_TRANSIENT);
  check(ret);
}

// Bind a binary blob value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV"
// in the SQL prepared statement
void Statement::bind(const int aIndex, const void* apValue, const int aSize) {
  const int ret =
      sqlite3_bind_blob(mStmtPtr, aIndex, apValue, aSize, SQLITE_TRANSIENT);
  check(ret);
}
...
void Statement::bindNoCopy(const int aIndex, const void* apValue,
                           const int aSize) {
  const int ret =
      sqlite3_bind_blob(mStmtPtr, aIndex, apValue, aSize, SQLITE_STATIC);
  check(ret);
}

// Bind a NULL value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in the
// SQL prepared statement
...
// Bind an int value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL
// prepared statement
...
// Bind a 32bits unsigned int value to a parameter "?NNN", ":VVV", "@VVV" or
...
// Bind a 64bits int value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in
// the SQL prepared statement
...
// Bind a double (64bits float) value to a parameter "?NNN", ":VVV", "@VVV" or
// "$VVV" in the SQL prepared statement
...
// Bind a string value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the
// SQL prepared statement
void Statement::bind(const char* apName, const std::string& aValue) {
  const int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
  const int ret =
      sqlite3_bind_text(mStmtPtr, index, aValue.c_str(),
                        static_cast<int>(aValue.size()), SQLITE_TRANSIENT);
  check(ret);
}

// Bind a text value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL
// prepared statement
void Statement::bind(const char* apName, const char* apValue) {
  const int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
  const int ret =
      sqlite3_bind_text(mStmtPtr, index, apValue, -1, SQLITE_TRANSIENT);
  check(ret);
}

// Bind a binary blob value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in
// the SQL prepared statement
void Statement::bind(const char* apName, const void* apValue, const int aSize) {
  const int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
  const int ret =
      sqlite3_bind_blob(mStmtPtr, index, apValue, aSize, SQLITE_TRANSIENT);
  check(ret);
}
...
// Bind a binary blob value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in
// the SQL prepared statement
void Statement::bindNoCopy(const char* apName, const void* apValue,
                           const int aSize) {
  const int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
  const int ret =
      sqlite3_bind_blob(mStmtPtr, index, apValue, aSize, SQLITE_STATIC);
  check(ret);
}

// Bind a NULL value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL
// prepared statement
...
int Statement::exec() {
  ...
  // Return the number of rows modified by those SQL statements (INSERT, UPDATE
  // or DELETE)
...
// Return the extended numeric result code for the most recent failed API call
// (if any).
int Statement::getExtendedErrorCode() const noexcept  // nothrow
{
  return sqlite3_extended_errcode(mStmtPtr);
}
// Return UTF-8 encoded English language explanation of the most recent failed
// API call (if any).
...
Statement::Ptr::Ptr(sqlite3* apSQLite, std::string& aQuery)
  ...
  const int ret = sqlite3_prepare_v2(
      apSQLite, aQuery.c_str(), static_cast<int>(aQuery.size()), &mpStmt, NULL);
  if (SQLITE_OK != ret) {
  ...
  // This is needed to enable Column objects to live longer than the Statement
  // objet it refers to.
...
Statement::Ptr::Ptr(const Statement::Ptr& aPtr)
  ...
  // asking not to finalize the sqlite3_stmt during the lifetime of the new
  // objet
  ++(*mpRefCount);
}

/**
 * @brief Decrement the ref counter and finalize the sqlite3_stmt when it
 * reaches 0
 */
Statement::Ptr::~Ptr() {
  assert(NULL != mpRefCount);
  ...
  --(*mpRefCount);
  if (0 == *mpRefCount) {
    // If count reaches zero, finalize the sqlite3_stmt, as no Statement nor
    // Column objet use it anymore. No need to check the return code, as it is
    // the same as the last statement evaluation.
