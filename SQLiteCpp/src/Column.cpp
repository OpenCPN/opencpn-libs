...
// Return the 64bits integer value of the column specified by its index starting
// at 0
...
const char* Column::getText(
    const char* apDefaultValue /* = "" */) const noexcept  // nothrow
{
  const char* pText =
      reinterpret_cast<const char*>(sqlite3_column_text(mStmtPtr, mIndex));
...
std::string Column::getString() const {
  ...
  const char* data =
      static_cast<const char*>(sqlite3_column_blob(mStmtPtr, mIndex));

  // SQLite docs: "The safest policy is to invoke… sqlite3_column_blob()
  // followed by sqlite3_column_bytes()" Note: std::string is ok to pass nullptr
  // as first arg, if length is 0
