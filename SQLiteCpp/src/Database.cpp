...
Database::Database(const char* apFilename,
    ...
    : mpSQLite(nullptr), mFilename(apFilename) {
  const int ret = sqlite3_open_v2(apFilename, &mpSQLite, aFlags, apVfs);
  if (SQLITE_OK != ret) {
    const SQLite::Exception exception(mpSQLite,
                                      ret);  // must create before closing
    sqlite3_close(
        mpSQLite);  // close is required even in case of error on opening
    throw exception;
  }
  if (aBusyTimeoutMs > 0) {
...
Database::Database(const std::string& aFilename,
    ...
    : mpSQLite(nullptr), mFilename(aFilename) {
 ...
 * locked for writting by a thread. Any other thread cannot access the table and
 * will receive a SQLITE_BUSY error: setting a timeout will wait and retry up to
 * the time specified before returning this SQLITE_BUSY error. Reading the value
 * of timeout for current connection can be done with SQL query "PRAGMA
 * busy_timeout;". Default busy timeout is 0ms.
 *
 * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before returning
 * SQLITE_BUSY
 *
...
// Shortcut to execute one or multiple SQL statements without results (UPDATE,
// INSERT, ALTER, COMMIT, CREATE...).
...
// result. WARNING: Be very careful with this dangerous method: you have to make
// a COPY OF THE result, else it will be destroy before the next line (when the
// underlying temporary Statement and Column objects are destroyed) this is an
// issue only for pointer type result (ie. char* and blob) (use the Column
// copy-constructor)
Column Database::execAndGet(const char* apQuery) {
  Statement query(*this, apQuery);
  (void)query.executeStep();  // Can return false if no result, which will throw
                              // next line in getColumn()
...
bool Database::tableExists(const char* apTableName) {
  ...
  (void)query.executeStep();  // Cannot return false, as the above query always
                              // return a result
  return (1 == query.getColumn(0).getInt());
}

// Get the rowid of the most recent successful INSERT into the database from the
// current connection.
...
void Database::createFunction(
  ...
  // optimization if deterministic function (e.g. of nondeterministic function
  // random())
  if (abDeterministic) {
    TextRep = TextRep | SQLITE_DETERMINISTIC;
  }
  const int ret =
      sqlite3_create_function_v2(mpSQLite, apFuncName, aNbArg, TextRep, apApp,
...
void Database::loadExtension(const char* apExtensionName,
  ...
  // Security warning:
  // It is recommended that the SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION method be
  // used to enable only this interface. The use of the
  // sqlite3_enable_load_extension() interface should be avoided to keep the SQL
  // load_extension() disabled and prevent SQL injections from giving attackers
  // access to extension loading capabilities. (NOTE: not using nullptr: cannot
  // pass object of non-POD type 'std::__1::nullptr_t' through variadic
  // function)
  int ret = sqlite3_db_config(mpSQLite, SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION,
