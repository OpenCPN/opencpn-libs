...
public:
   ...
   * Object.
   * @param[in] aIndex    Index of the column in the row of result, starting at
   * 0
   */
  ...
  // they copy the Statement::Ptr which in turn increments the reference
  // counter.
  ...
  /**
   * @brief Return a pointer to the table column name that is the origin of this
   * result column
   *
  ...
  /// Return the 32bits unsigned integer value of the column (note that SQLite3
  /// does not support unsigned 64bits).
  unsigned getUInt() const noexcept;  // nothrow
  /// Return the 64bits integer value of the column (note that SQLite3 does not
  /// support unsigned 64bits).
  long long getInt64() const noexcept;  // nothrow
  /// Return the double (64bits float) value of the column
  double getDouble() const noexcept;  // nothrow
  /**
   * @brief Return a pointer to the text value (NULL terminated string) of the
   * column.
   *
   * @warning The value pointed at is only valid while the statement is valid
   * (ie. not finalized), thus you must copy it before using it beyond its scope
   * (to a std::string for instance).
   */
  const char* getText(
      const char* apDefaultValue = "") const noexcept;  // nothrow
  /**
   ...
   *
   * Return either SQLite::INTEGER, SQLite::FLOAT, SQLite::TEXT, SQLite::BLOB,
   * or SQLite::Null.
   *
  ...
  /// Test if the column is a floating point type value (meaningful only before
  /// any conversion)
  ...
  /// Test if the column is a binary blob type value (meaningful only before any
  /// conversion)
  ...
  /**
   * @brief Return the number of bytes used by the text (or blob) value of the
   * column
   *
   * Return either :
  ...
  inline operator unsigned int() const { return getUInt(); }
  ...
  inline operator long() const { return getInt(); }
  /// Inline cast operator to 32bits unsigned long
  inline operator unsigned long() const { return getUInt(); }
#else
  /// Inline cast operator to 64bits long when the data model of the system is
  /// ILP64 (Linux 64 bits...)
  inline operator long() const { return getInt64(); }
#endif

  /// Inline cast operator to 64bits integer
  inline operator long long() const { return getInt64(); }
   ...
   * @see getText
   */
  inline operator const char*() const { return getText(); }
  /**
   * @brief Inline cast operator to void*
   *
   * @see getBlob
   */
  inline operator const void*() const { return getBlob(); }

#if !(defined(_MSC_VER) && _MSC_VER < 1900)
  // NOTE : the following is required by GCC and Clang to cast a Column result
  // in a std::string (error: conversion from ‘SQLite::Column’ to non-scalar
  // type ‘std::string {aka std::basic_string<char>}’) but is not working under
  // Microsoft Visual Studio 2010, 2012 and 2013 (error C2440: 'initializing' :
  // cannot convert from 'SQLite::Column' to
