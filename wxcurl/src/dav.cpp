...
bool wxCurlDAV::Propfind(const wxArrayString& arrTags,
...
bool wxCurlDAV::Propfind(const char* buffer, size_t size,
                         const wxString& szRemoteLoc /*= wxEmptyString*/) {
...
bool wxCurlDAV::Propfind(wxInputStream& buffer,
                         const wxString& szRemoteLoc /*= wxEmptyString*/) {
  curl_off_t iSize = buffer.GetSize();

  if (m_pCURL && buffer.IsOk()) {
    if (iSize == (~(ssize_t)0))  // wxCurlDAV does not know how to upload
...
bool wxCurlDAV::Proppatch(const char* buffer, size_t size,
                          const wxString& szRemoteLoc /*= wxEmptyString*/) {
  wxMemoryInputStream inStream(buffer, size);
...
bool wxCurlDAV::Proppatch(wxInputStream& buffer,
                          const wxString& szRemoteLoc /*= wxEmptyString*/) {
  curl_off_t iSize = buffer.GetSize();
...
bool wxCurlDAV::Copy(const wxString& szRemoteLocDest,
  ...
  if (m_pCURL) {
    ...
    if (!bOverwrite) m_arrHeaders.Add(wxS("Overwrite: F"));
...
bool wxCurlDAV::Move(const wxString& szRemoteLocDest,
                     const bool& bOverwrite /*= true*/,
                     const wxString& szRemoteLocSrc /*= wxEmptyString*/) {
  if (m_pCURL) {
    SetCurlHandleToDefaults(szRemoteLocSrc);
...
bool wxCurlDAV::Unlock(
    const wxString& WXUNUSED(szRemoteLoc /*=wxEmptyString*/)) {
  return false;
}
