...
extern "C" {
int wxcurl_size_query_progress_func(void *ptr, double rDlTotal,
                                    double WXUNUSED(rDlNow),
                                    double WXUNUSED(rUlTotal),
                                    double WXUNUSED(rUlNow)) {
...
void *wxCurlSizeQueryThread::Entry() {
  ...
  wxLogDebug(
      wxS("wxSizeCacherThread::Entry - caching of file sizes completed"));
  return (void *)allok;
}

void wxCurlSizeQueryThread::
    OnExit() { /*
                  ...
                  // the event handler must delete the wxArrayLong which we pass
                  to it in the event ! updatevent.SetClientData(m_urlSizes); if
                  (m_pHandler) wxPostEvent(m_pHandler, updatevent);*/
}
