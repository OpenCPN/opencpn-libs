...
wxCurlFTP::wxCurlFTP(const wxString& szURL /*= wxEmptyString*/,
    ...
    : wxCurlBase(szURL, szUserName, szPassword, pEvtHandler, id, flags),
      ...
      m_pPreQuote(NULL),
      m_pPostQuote(NULL),
      m_bCreateMissingDirs(true),
      m_bAppend(false),
      m_bUsePortOption(false),
      m_tmMode(kASCII),
      m_bUseEPRT(false),
      m_bUseEPSV(false),
      m_szPortParam("-") {}

wxCurlFTP::~wxCurlFTP() { ResetAllQuoteLists(); }
...
wxCurlFTP::TransferMode wxCurlFTP::GetTransferMode() const { return m_tmMode; }

void wxCurlFTP::SetToBinary() { SetTransferMode(kBINARY); }

void wxCurlFTP::SetToAscii() { SetTransferMode(kASCII); }

bool wxCurlFTP::IsAscii() const { return m_tmMode == kASCII; }

bool wxCurlFTP::IsBinary() const { return m_tmMode == kBINARY; }
...
void wxCurlFTP::UseEPRT(const bool& bUseEPRT) { m_bUseEPRT = bUseEPRT; }

bool wxCurlFTP::UseEPRT() const { return m_bUseEPRT; }

void wxCurlFTP::UseEPSV(const bool& bUseEPSV) { m_bUseEPSV = bUseEPSV; }

bool wxCurlFTP::UseEPSV() const { return m_bUseEPSV; }

void wxCurlFTP::AppendOnUpload(const bool& bAppend) { m_bAppend = bAppend; }

bool wxCurlFTP::AppendOnUpload() const { return m_bAppend; }

void wxCurlFTP::CreateMissingDirs(const bool& bCreateMissing) {
  m_bCreateMissingDirs = bCreateMissing;
}

bool wxCurlFTP::CreateMissingDirs() const { return m_bCreateMissingDirs; }
...
bool wxCurlFTP::AppendQuote(const wxString& szQuote,
                            const bool& bClear /*= false*/) {
  if (bClear) ResetQuoteList();
...
bool wxCurlFTP::SetPreQuoteList(const wxArrayString& arrQuote) {
  ...
  for (unsigned int i = 0; i < arrQuote.Count(); i++) {
    m_pPreQuote =
        curl_slist_append(m_pPreQuote, (const char*)(arrQuote[i].c_str()));
  }

  return (m_pPreQuote != NULL);
}

bool wxCurlFTP::AppendPreQuote(const wxString& szQuote,
                               const bool& bClear /*= false*/) {
  if (bClear) ResetPreQuoteList();
...
bool wxCurlFTP::SetPostQuoteList(const wxArrayString& arrQuote) {
  ...
  for (unsigned int i = 0; i < arrQuote.Count(); i++) {
    m_pPostQuote =
        curl_slist_append(m_pPostQuote, (const char*)(arrQuote[i].c_str()));
  }

  return (m_pPostQuote != NULL);
}

bool wxCurlFTP::AppendPostQuote(const wxString& szQuote,
                                const bool& bClear /*= false*/) {
  if (bClear) ResetPostQuoteList();

  m_pPostQuote =
      curl_slist_append(m_pPostQuote, (const char*)(szQuote.c_str()));
...
bool wxCurlFTP::Get(const wxString& szFilePath,
...
size_t wxCurlFTP::Get(char*& buffer,
                      const wxString& szRemoteFile /*= wxEmptyString*/) {
...
bool wxCurlFTP::Get(wxOutputStream& buffer,
                    const wxString& szRemoteFile /*= wxEmptyString*/) {
...
bool wxCurlFTP::Put(const wxString& szFilePath,
                    const wxString& szRemoteFile /*= wxEmptyString*/) {
...
bool wxCurlFTP::Put(const char* buffer, size_t size,
                    const wxString& szRemoteFile /*= wxEmptyString*/) {
...
bool wxCurlFTP::Put(wxInputStream& buffer,
                    const wxString& szRemoteFile /*= wxEmptyString*/) {
  curl_off_t iSize = 0;
  ...
  if (m_pCURL && buffer.IsOk()) {
    ...
    if (iSize == (~(ssize_t)0)) return false;
...
bool wxCurlFTP::RmDir(const wxString& szRemoteLoc /*= wxEmptyString*/) {
  if (m_pCURL) {
    ...
    if (m_szCurrFilename.IsEmpty()) return false;
...
bool wxCurlFTP::Delete(const wxString& szRemoteLoc /*= wxEmptyString*/) {
  if (m_pCURL) {
    ...
    if (m_szCurrFilename.IsEmpty()) return RmDir(szRemoteLoc);
