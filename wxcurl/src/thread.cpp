...
/* static */
std::shared_ptr<wxCurlBase> wxCurlBaseThread::CreateHandlerFor(
    wxCurlProtocol prot) {
  switch (prot) {
    case wxCP_HTTP:
      return std::shared_ptr<wxCurlBase>(
          new wxCurlHTTP);  // std::make_shared<wxCurlHTTP>();
    case wxCP_FTP:
      return std::shared_ptr<wxCurlBase>(
          new wxCurlFTP);  // std::make_shared<wxCurlFTP>();
    default:
...
wxCurlThreadError wxCurlBaseThread::SetURL(const wxString &url,
  ...
  wxCHECK_MSG(!IsAlive(), wxCTE_NO_RESOURCE,
  ...
  if (!m_pCurl || !m_pCurl->IsOk()) return wxCTE_INVALID_PROTOCOL;
...
void wxCurlBaseThread::OnExit() {
  if (m_pCurl->IsVerbose()) wxLogDebug(wxS("wxCurlBaseThread - exiting"));
}

bool wxCurlBaseThread::TestDestroy() {
  if (wxThread::TestDestroy()) return true;
...
size_t wxCurlDownloadThreadOutputFilter::OnSysWrite(const void *buffer,
                                                    size_t bufsize) {
  // VERY IMPORTANT: we need to call TestDestroy() on the associated
  //                 thread to make sure it can be paused/stopped (see wxThread
  //                 docs)

  if (m_thread->TestDestroy()) {
    if (m_thread->m_pCurl->IsVerbose())
      wxLogDebug(
          wxS("[wxCURL] a wxCurlDownloadThread has been aborted - ignore "
              "following message:"));

    // returning a size != bufsize we tell libcurl to stop the tranfer
    // and thus the wxCurlBase::Perform() call done (indirectly) by
    // wxCurlDownloadThread::Entry() will immediately complete and we'll exit
    // the thread. NOTE: we could also call wxThread::Exit() from here... but
    // notifying libcurl
    //       of a "fake error" seems a better solution (allows libcurl to
    //       cleanup its internal)
    return (size_t)-1;
  }
...
wxCurlThreadError wxCurlDownloadThread::SetOutputStream(wxOutputStream *out) {
  wxCHECK_MSG(!IsAlive(), wxCTE_NO_RESOURCE,
              wxS("Cannot use this function after the transfer has begun"));

  if (!out) {
    if (!m_output) {
      // create a temporary file output stream
      // NOTE: by default we create a wxFileOutputStream and not e.g. a
      // wxMemoryOutputStream
      //       because the downloaded file may be quite large and thus we don't
      //       want to eat lots of RAM. Also, the hard disk should be fast
      //       enough for any download.
      m_output.SetStream(new wxFileOutputStream(
          wxFileName::CreateTempFileName(wxS("download"))));
      if (!m_output.IsOk()) return wxCTE_NO_VALID_STREAM;
    }
  } else {
...
wxCurlThreadError wxCurlDownloadThread::Download() {
  ...
  if ((ret = Create(wxCURL_THREAD_STACK_SIZE)) != wxCTE_NO_ERROR) return ret;
  if ((ret = Run()) != wxCTE_NO_ERROR) return ret;
...
wxCurlThreadError wxCurlDownloadThread::Download(const wxString &url,
  ...
  if ((ret = SetURL(url)) != wxCTE_NO_ERROR) return ret;
  if ((ret = SetOutputStream(out)) != wxCTE_NO_ERROR) return ret;

  return Download();
}

void *wxCurlDownloadThread::Entry() {
  if (!m_pCurl || !m_pCurl->IsOk() || !m_output.IsOk()) return NULL;
  ...
  if (m_pCurl->IsVerbose())
    wxLogDebug(wxS("wxCurlDownloadThread - downloading from %s"),
               m_url.c_str());
  switch (m_protocol) {
    case wxCP_HTTP:
      return (void *)wx_static_cast(wxCurlHTTP *, m_pCurl.get())
          ->Get(m_output, m_url);

    case wxCP_FTP:
      return (void *)wx_static_cast(wxCurlFTP *, m_pCurl.get())
          ->Get(m_output, m_url);
...
size_t wxCurlUploadThreadInputFilter::OnSysRead(void *buffer, size_t bufsize) {
  ...
  if (m_thread->TestDestroy()) {
    if (m_thread->m_pCurl->IsVerbose())
      wxLogDebug(
          wxS("[wxCURL] a wxCurlUploadThread has been aborted - ignore "
              "following message:"));

    // returning a size != bufsize we tell libcurl to stop the tranfer
    // and thus the wxCurlBase::Perform() call done (indirectly) by
    // wxCurlUploadThread::Entry() will immediately complete and we'll exit the
    // thread. NOTE: we could also call wxThread::Exit() from here... but
    // notifying libcurl
    //       of a "fake error" seems a better solution (allows libcurl to
    //       cleanup its internal)
    return (size_t)-1;
...
wxCurlThreadError wxCurlUploadThread::SetInputStream(wxInputStream *in) {
  ...
  if (!m_input.IsOk()) return wxCTE_NO_VALID_STREAM;
...
wxCurlThreadError wxCurlUploadThread::Upload(const wxString &url,
  ...
  if ((ret = SetURL(url)) != wxCTE_NO_ERROR) return ret;
  if ((ret = SetInputStream(in)) != wxCTE_NO_ERROR) return ret;

  return Upload();
}

void *wxCurlUploadThread::Entry() {
  if (!m_pCurl || !m_pCurl->IsOk() || !m_input.IsOk()) return NULL;
