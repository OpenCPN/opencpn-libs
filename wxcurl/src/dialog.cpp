...
EVT_UPDATE_UI(ConnSettingsButtonId,
              wxCurlTransferDialog::OnConnSettingsUpdateUI)
...
bool wxCurlTransferDialog::Create(const wxString &url, const wxString &title,
                                  const wxString &message,
                                  const wxString &sizeLabel,
  ...
  wxASSERT_MSG(HasFlag(wxCTDS_AUTO_CLOSE) || HasFlag(wxCTDS_CAN_ABORT),
               wxS("If both these flags are missing the user will be unable to "
  ...
  wxASSERT_MSG(!(HasFlag(wxCTDS_CONN_SETTINGS_AUTH) ||
                 ...
                 HasFlag(wxCTDS_CONN_SETTINGS_PROXY)) ||
                   HasFlag(wxCTDS_CAN_START),
               wxS("the connection settings may only be changed before the "
                   "transfer starts; if wxCTDS_CAN_START ")
                   wxS("is missing the user will be unable to use the "
...
wxStaticText *wxCurlTransferDialog::AddSizerRow(wxSizer *sz,
                                                const wxString &name) {
  // the static text
  wxStaticText *st = new wxStaticText(this, wxID_STATIC, name,
                                      wxDefaultPosition, wxDefaultSize);
  st->SetFont(wxFont(8, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL,
                     wxFONTWEIGHT_BOLD, false, wxS("")));

  // the value
  wxStaticText *ret =
      new wxStaticText(this, wxID_STATIC, _("Not available"), wxDefaultPosition,
...
void wxCurlTransferDialog::CreateControls(const wxString &url,
                                          const wxString &msg,
                                          const wxString &sizeLabel,
  ...
  if (!msg.IsEmpty()) {
    ...
    main->Add(st, 0, wxLEFT | wxTOP | wxRIGHT | wxBOTTOM | wxGROW,
              OUTER_BORDER);
  }
  ...
  if (HasFlag(wxCTDS_URL)) {
    ...
    m_pURL =
        new wxStaticText(this, wxID_STATIC, url, wxDefaultPosition,
  ...
  // speed & size row
  if (HasFlag(wxCTDS_SPEED)) m_pSpeed = AddSizerRow(leftcolumn, _("Speed:"));
  if (HasFlag(wxCTDS_SIZE)) m_pSize = AddSizerRow(leftcolumn, sizeLabel);
  ...
  if (bitmap.IsOk()) {
    ...
    rightcolumn->Add(new wxStaticBitmap(this, wxID_ANY, bitmap), 0,
                     wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, BORDER);
  ...
  // the gauge
  // VERY IMPORTANT: we set as range 101 so that when, because of some
  // approximation,
  //                 the update event handler will SetValue(100), while the
  //                 transfer is not yet complete, the gauge will not appear
  //                 completely filled.
  ...
  if (HasFlag(wxCTDS_CAN_ABORT) || HasFlag(wxCTDS_CAN_PAUSE) ||
      HasFlag(wxCTDS_CAN_START) || needsConnSettings) {
    main->AddStretchSpacer(1);
    main->AddSpacer(BORDER * 2);
    main->Add(new wxStaticLine(this), 0, wxGROW | wxLEFT | wxRIGHT,
              OUTER_BORDER);
    ...
    if (HasFlag(wxCTDS_CAN_ABORT))
      btn->Add(new wxButton(this, AbortButtonId, _("Abort")), 0, wxRIGHT,
               BORDER);
    ...
    if (HasFlag(wxCTDS_CAN_START))
      btn->Add(new wxButton(this, StartButtonId, _("Start")), 0, wxLEFT,
               BORDER);

    btn->SetMinSize(
        wxSize(-1, wxButton::GetDefaultSize().GetHeight() + 2 * OUTER_BORDER));

    main->Add(btn, 0, wxGROW | wxLEFT | wxRIGHT | wxTOP | wxBOTTOM,
              OUTER_BORDER);
  }
...
void wxCurlTransferDialog::UpdateLabels(wxCurlProgressBaseEvent *ev) {
  ...
  if (ev->GetElapsedTime().IsShorterThan(wxTimeSpan::Minutes(120))) {
    if (m_pElapsedTime) m_pElapsedTime->SetLabel(ev->GetElapsedTime().Format());
    if (m_pRemainingTime)
  ...
  if (m_pSize) {
    wxString currsize(ev->GetHumanReadableTransferredBytes().c_str(),
                      wxConvUTF8);
    wxString totalsize(ev->GetHumanReadableTotalBytes().c_str(), wxConvUTF8);
    m_pSize->SetLabel(wxString::Format(wxS("%s / %s  (%0.1f%%)"),
                                       currsize.c_str(), totalsize.c_str(),
                                       ev->GetPercent()));
  }
...
bool wxCurlTransferDialog::HandleCurlThreadError(wxCurlThreadError err,
                                                 wxCurlBaseThread *p,
                                                 const wxString &url) {
  switch (err) {
    case wxCTE_NO_ERROR:
...
void wxCurlTransferDialog::OnAbort(wxCommandEvent &WXUNUSED(ev)) {
  // NOTE: the wxCTDS_ABORT flag may be absent if the user
  // wxASSERT(HasFlag(wxCTDS_CAN_ABORT));
...
void wxCurlTransferDialog::OnPauseResume(wxCommandEvent &WXUNUSED(ev)) {
  ...
  if (m_pThread->IsRunning()) {
    if (HandleCurlThreadError(m_pThread->Pause(), m_pThread)) {
      ...
      if (m_pSpeed) m_pSpeed->SetLabel(_("0 (transfer paused)"));
    }
  } else {
...
void wxCurlTransferDialog::OnStart(wxCommandEvent &WXUNUSED(ev)) {
  // wxASSERT(HasFlag(wxCTDS_CAN_START));      // OnStart() must be usable also
  // from RunModal() when !HasFlag(wxCTDS_CAN_START)
...
void wxCurlTransferDialog::OnConnSettings(wxCommandEvent &WXUNUSED(ev)) {
  ...
  if (HasFlag(wxCTDS_CONN_SETTINGS_PORT)) style |= wxCCSP_PORT_OPTION;
  if (HasFlag(wxCTDS_CONN_SETTINGS_PROXY)) style |= wxCCSP_PROXY_OPTIONS;

  wxCurlConnectionSettingsDialog dlg(
      _("Connection settings"), _("Connection settings used for the transfer:"),
      this, style);
...
void wxCurlTransferDialog::OnEndPerform(wxCurlEndPerformEvent &ev) {
  ...
  if (m_pLastEvent) UpdateLabels(m_pLastEvent);
  // else: we may have transferred a file of zero size and thus have never
  // received any progress event...
  ...
  wxCurlDialogReturnFlag retCode =
      ev.IsSuccessful() ? wxCDRF_SUCCESS : wxCDRF_FAILED;
#if 0  // below is bugged because m_pThread->GetCurlSession() is NULL at this
       // point...
  ...
  if (HasFlag(wxCTDS_AUTO_CLOSE))
    ...
    if (m_pSpeed) m_pSpeed->SetLabel(_("0 (transfer completed)"));
  }
}
...
bool wxCurlDownloadDialog::Create(const wxString &url, wxOutputStream *out,
                                  ...
                                  const wxBitmap &bitmap, wxWindow *parent,
                                  long style) {
  if (!wxCurlTransferDialog::Create(url, title, message, _("Downloaded:"),
                                    bitmap, parent, style))
    return false;
  ...
  if (!HandleCurlThreadError(thread->SetURL(url), thread, url)) return false;
...
void wxCurlDownloadDialog::OnDownload(wxCurlDownloadEvent &ev) {
  static wxDateTime lastLabelUpdate(
      0, 0, 0, 0);  // zero is to force always at least an update
  if ((wxDateTime::Now() - lastLabelUpdate).GetMilliseconds() >
      200)  // avoid flickering
  {
    UpdateLabels(&ev);
...
bool wxCurlUploadDialog::Create(const wxString &url, wxInputStream *in,
                                ...
                                const wxBitmap &bitmap, wxWindow *parent,
                                long style) {
  if (!wxCurlTransferDialog::Create(url, title, message, _("Uploaded:"), bitmap,
                                    parent, style))
    return false;
  ...
  if (!HandleCurlThreadError(thread->SetURL(url), thread, url)) return false;
  if (!HandleCurlThreadError(thread->SetInputStream(in), thread)) return false;
...
bool wxCurlConnectionSettingsDialog::Create(const wxString &title,
  ...
  m_pPanel = new wxCurlConnectionSettingsPanel(
      this, wxID_ANY, message, wxDefaultPosition, wxDefaultSize, style);
...
void wxCurlConnectionSettingsDialog::RunModal(wxCurlBase *pCURL) {
  if (ShowModal() == wxID_OK) m_pPanel->SetCURLOptions(pCURL);
}
