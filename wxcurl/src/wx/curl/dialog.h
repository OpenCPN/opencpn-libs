...
enum wxCurlTransferDialogStyle {
  ...
  wxCTDS_SIZE = 0x0010,  //!< The dialog shows the size of the resource to
                         //!< download/upload.
  ...
  wxCTDS_CONN_SETTINGS_AUTH = 0x0040,  //!< The dialog allows the user to change
                                       //!< the authentication settings.
  wxCTDS_CONN_SETTINGS_PORT = 0x0080,  //!< The dialog allows the user to change
                                       //!< the port for the transfer.
  wxCTDS_CONN_SETTINGS_PROXY =
      0x0100,  //!< The dialog allows the user to change the proxy settings.

  wxCTDS_CONN_SETTINGS_ALL = wxCTDS_CONN_SETTINGS_AUTH |
                             wxCTDS_CONN_SETTINGS_PORT |
                             wxCTDS_CONN_SETTINGS_PROXY,

  wxCTDS_SHOW_ALL = wxCTDS_ELAPSED_TIME | wxCTDS_ESTIMATED_TIME |
                    wxCTDS_REMAINING_TIME | wxCTDS_SPEED | wxCTDS_SIZE |
                    wxCTDS_URL | wxCTDS_CONN_SETTINGS_ALL,

  wxCTDS_CAN_ABORT = 0x0200,  //!< The transfer can be aborted by the user.
  wxCTDS_CAN_START = 0x0400,  //!< The transfer won't start automatically. The
                              //!< user needs to start it.
  ...
  wxCTDS_DEFAULT_STYLE = wxCTDS_CAN_START | wxCTDS_CAN_PAUSE |
                         wxCTDS_CAN_ABORT | wxCTDS_SHOW_ALL | wxCTDS_AUTO_CLOSE
};
...
public:
  ...
  bool Create(const wxString &url, const wxString &title,
              const wxString &message = wxEmptyString,
              const wxString &sizeLabel = _("Transferred:"),
              const wxBitmap &bitmap = wxNullBitmap, wxWindow *parent = NULL,
  ...
  */
  ...
  void SetVerbose(bool enable) { m_bVerbose = enable; }

  //! Returns true if the internal wxCurlBase is verbose (on by default in debug
  //! builds).
...
protected:  // internal utils
  ...
  virtual int ShowModal() { return wxDialog::ShowModal(); }
...
protected:
  ...
  // wxWindow's style member is too small for all our flags and
  // wxWindow/wxDialog ones. So we use our own...
  long m_nStyle;
...
public:
  ...
  wxCurlDownloadDialog(const wxString &url, wxOutputStream *out,
  ...
  bool Create(const wxString &url, wxOutputStream *out,
              const wxString &title = wxS("Downloading..."),
              const wxString &message = wxEmptyString,
              const wxBitmap &bitmap = wxNullBitmap, wxWindow *parent = NULL,
              long style = wxCTDS_DEFAULT_STYLE);
...
public:
  ...
  wxCurlUploadDialog(const wxString &url, wxInputStream *in,
  ...
  bool Create(const wxString &url, wxInputStream *in,
              const wxString &title = wxS("Uploading..."),
              const wxString &message = wxEmptyString,
              const wxBitmap &bitmap = wxNullBitmap, wxWindow *parent = NULL,
              long style = wxCTDS_DEFAULT_STYLE);
...
public:
  ...
  bool Create(const wxString &title = wxS("Connection settings..."),
              const wxString &message = wxEmptyString, wxWindow *parent = NULL,
              long style = wxCCSP_DEFAULT_STYLE);

public:
  //! Runs this dialog as modal and updates the given wxCurlBase instance if
  //! needed (i.e. if the user hits OK and not Cancel).
