...
//! Private internal class used as base class for wxCurlDownloadEvent and
//! wxCurlUploadEvent.
class WXDLLIMPEXP_CURL wxCurlProgressBaseEvent : public wxEvent {
public:
  wxCurlProgressBaseEvent(int id, wxEventType type, wxCurlBase* p = NULL,
...
public:  // misc getters
  ...
  //! Returns a number in [0;100] range indicating how much has been transferred
  //! so far.
  double GetPercent() const {
    return GetTotalBytes() == 0
               ? 0
               : (100.0 * (GetTransferredBytes() / GetTotalBytes()));
  }
...
public:  // wxString getters
  ...
  std::string GetHumanReadableSpeed(const std::string& inv = GetNAText(),
                                    int prec = 1) const;

  //! Returns the total bytes to download in a human-readable format.
  std::string GetHumanReadableTotalBytes(const std::string& inv = GetNAText(),
                                         int prec = 1) const {
    wxString s(inv.c_str(), wxConvUTF8);
    return std::string(wxFileName::GetHumanReadableSize(
                           wxULongLong((unsigned long)GetTotalBytes()), s, prec)
                           .mb_str());
  }

  //! Returns the currently transferred bytes in a human-readable format.
  std::string GetHumanReadableTransferredBytes(
      const std::string& inv = GetNAText(), int prec = 1) const {
    wxString s(inv.c_str(), wxConvUTF8);
    return std::string(
        wxFileName::GetHumanReadableSize(
            wxULongLong((unsigned long)GetTransferredBytes()), s, prec)
            .mb_str());
  }
...
protected:
  ...
  //       we cannot use wxDateTime::Now() there because once the event is
  //       constructed, GetElapsedTime() needs to return always the same value!
  wxDateTime m_dt;
...
public:
  ...
  std::string GetHumanReadableDownloadedBytes(
      const std::string& inv = wxCurlProgressBaseEvent::GetNAText(),
      int prec = 1) const {
    wxString s(inv.c_str(), wxConvUTF8);
    return std::string(wxFileName::GetHumanReadableSize(
                           wxULongLong((unsigned long)m_rDownloadNow), s, prec)
                           .mb_str());
  }
...
#define wxCurlDownloadEventHandler(func)   \
  (wxObjectEventFunction)(wxEventFunction) \
      wxStaticCastEvent(wxCurlDownloadEventFunction, &func)
...
public:
  wxCurlUploadEvent();
  wxCurlUploadEvent(int id, wxCurlBase* originator, const double& rUploadTotal,
                    const double& rUploadNow, const std::string& szURL = "");
  ...
  std::string GetHumanReadableUploadedBytes(
      const std::string& inv = wxCurlProgressBaseEvent::GetNAText(),
      int prec = 1) const {
    wxString s(inv.c_str(), wxConvUTF8);
    return std::string(wxFileName::GetHumanReadableSize(
                           wxULongLong((unsigned long)m_rUploadNow), s, prec)
                           .mb_str());
  }
...
#define wxCurlUploadEventHandler(func)     \
  (wxObjectEventFunction)(wxEventFunction) \
      wxStaticCastEvent(wxCurlUploadEventFunction, &func)
...
#define wxCurlBeginPerformEventHandler(func) \
  (wxObjectEventFunction)(wxEventFunction)   \
      wxStaticCastEvent(wxCurlBeginPerformEventFunction, &func)

#define EVT_CURL_BEGIN_PERFORM(id, fn)             \
  wx__DECLARE_EVT1(wxCURL_BEGIN_PERFORM_EVENT, id, \
                   wxCurlBeginPerformEventHandler(fn))
...
public:
  ...
  wxCurlEndPerformEvent(int id, const std::string& szURL,
                        const long& iResponseCode);
...
typedef void (wxEvtHandler::*wxCurlEndPerformEventFunction)(
    wxCurlEndPerformEvent&);

#define wxCurlEndPerformEventHandler(func) \
  (wxObjectEventFunction)(wxEventFunction) \
      wxStaticCastEvent(wxCurlEndPerformEventFunction, &func)

#define EVT_CURL_END_PERFORM(id, fn)             \
  wx__DECLARE_EVT1(wxCURL_END_PERFORM_EVENT, id, \
                   wxCurlEndPerformEventHandler(fn))
...
int wxcurl_verbose_stream_write(CURL* crlptr, curl_infotype info,
                                char* cStrMessage, size_t msgSize,
                                void* buffer);
...
//! Tells wxCurlBase to send wxCurlBeginPerformEvent and wxCurlEndPerformEvent
//! events
...
//! You may want to look at wxCurlFTP, wxCurlHTTP, wxCurlDAV if you want to have
//! a specialized interface for respectively the FTP, HTTP and WebDAV protocols.
//!
...
public:
  ...
  //! Start the operation as described by the options set previously with
  //! #SetOpt. If you set CURLOPT_UPLOAD to zero and the CURLOPT_WRITEFUNCTION
  //! and CURLOPT_WRITEDATA options to suitable values, a download will be
  //! performed. If you set CURLOPT_UPLOAD to nonzero and the
  //! CURLOPT_READFUNCTION and CURLOPT_READDATA options to suitable values, an
  //! upload will be performed. See the curl_easy_perform() function call for
  //! more info.
  bool Perform();

  // Internal handle management:

  //! Initializes the internal libCURL handle. This function is automatically
  //! called by the constructor.
  bool InitHandle();

  //! Closes this libCURL session. This will effectively close all connections
  //! this handle has used and possibly has kept open until now. This function
  //! is automatically called by the destructor.
  bool CleanupHandle();

  //! Reinit the handle of this libCURL session. Equivalent to call
  //! #CleanupHandle and then #InitHandle.
  bool ReInitHandle();

  //! Re-initializes all options previously set on this libCURL session to the
  //! default values.
  bool ResetHandle();
  ...
  //! Sets the event handler to which the wxCurlDownloadEvent,
  //! wxCurlBeginPerformEvent and wxCurlEndPerformEvent will be sent if they are
  //! enabled (see #SetFlags).
  ...
  //! Sets the "event policy" of wxCURL: if you pass zero, then no events will
  //! ever be sent. The wxCURL_SEND_PROGRESS_EVENTS and
  //! wxCURL_SEND_BEGINEND_EVENTS flags instead tell wxCURL to send respectively
  //! the wxCurlDownloadEvent and wxCurlBeginPerformEvent, wxCurlEndPerformEvent
  //! events.
  ...
  //! Sets the current URL. The 'base url' will be prepended to the given
  //! string.
  void SetURL(const wxString& szRelativeURL);

  //! Returns the current 'full' URL. I.e. the real URL being used for the
  //! transfer.
  ...
  std::string GetResponseBody()
      const;  // May only contain data on NON-GET calls.
  ...
  //! Initializes the libCURL. Call this only once at the beginning of your
  //! program.
  static void Init();
...
protected:  // protected utils used by wxCurl*Thread classes
  ...
  void OverrideProgressCallback(curl_progress_callback newcallback,
                                void* data) {
  ...
  wxDateTime GetBeginTransferSpan() const { return m_dtBeginTransferSpan; }

  //! Returns the current time offset, i.e. the time elapsed in all previous
  //! transfer spans.
  wxTimeSpan GetElapsedTimeOffset() const { return m_tsElapsedOffset; }
  ...
  void BeginTransferSpan() { m_dtBeginTransferSpan = wxDateTime::Now(); }
...
#define wxCURL_BUF_ISEMPTY(x) (strlen(x) == 0)
  ...
  //                 in wxWidgets Unicode builds would use a wxWCharBuffer
  //                 internally. wxCharBuffer is basically a thin class which
  //                 handles auto-delete of a char[] array.
