...
int wxcurl_evt_progress_func(void* ptr, double rDlTotal, double rDlNow,
  ...
  if (curl) {
    ...
    if (curl->GetAbort())
      res = 1;  // This will cause curl_easy_perform() to return
                // CURLE_WRITE_ERROR immediately
  }
  return res;
}
...
int wxcurl_verbose_stream_write(CURL* crlptr, curl_infotype info,
  ...
  switch (info) {
    ...
    case CURLINFO_SSL_DATA_OUT:
      szVerboseMessage =
          wxString(wxS("SSL Data out: ")) + szMessage + wxS("\n");
      break;
  }

  // here wxChar usage is correct as szVerboseMessage string is used:
  pBuf->Write((const wxChar*)szVerboseMessage,
              szVerboseMessage.Len() * sizeof(wxChar));
...
size_t wxcurl_header_func(void* ptr, size_t size, size_t nmemb, void* stream) {
  ...
  if (pStr) {
    wxString str =
        wxCURL_BUF2STRING(*pStr) + wxString((const char*)ptr, wxConvLibc);
...
/* writes to a string */
size_t wxcurl_string_write(void* ptr, size_t size, size_t nmemb,
                           void* pcharbuf) {
  size_t iRealSize = size * nmemb;
...
/* reads from a string */
size_t wxcurl_string_read(void* ptr, size_t size, size_t nmemb,
                          void* pcharbuf) {
  size_t iRealSize = size * nmemb;
  size_t iRetVal = 0;
...
wxTimeSpan wxCurlProgressBaseEvent::GetElapsedTime() const {
  // NOTE: we cannot trust libCURL's CURLINFO_TOTAL_TIME as the transfer may
  // have
  //       been paused in one of libCURL's callbacks (and thus libCURL ignores
  //       it and won't remove the paused span from the return value).
  wxTimeSpan elapsed = m_dt - m_pCURL->GetBeginTransferSpan();

  // the elapsed time offset takes in count eventually-existing previous time
  // spans where the transfer took place (
...
wxTimeSpan wxCurlProgressBaseEvent::GetEstimatedRemainingTime() const {
  wxTimeSpan est = GetEstimatedTime(), elapsed = GetElapsedTime();

  if (est.IsLongerThan(elapsed)) return est - elapsed;
  return wxTimeSpan(0);  // probably est==0 because GetTotalBytes()==0
}

std::string wxCurlProgressBaseEvent::GetHumanReadableSpeed(
    const std::string& invalid, int precision) const {
  double speed = GetSpeed();
  if (speed == 0 || wxIsNaN(speed)) return invalid;

  wxULongLong ull((wxULongLong_t)speed);
  wxString s(invalid.c_str(), wxConvUTF8);
  return std::string(
             wxFileName::GetHumanReadableSize(ull, s, precision).mb_str()) +
         "/s";
}
...
wxCurlDownloadEvent::wxCurlDownloadEvent(
    int id, wxCurlBase* originator, const double& rDownloadTotal,
    const double& rDownloadNow, const std::string& szURL /*= wxEmptyString*/)
    : wxCurlProgressBaseEvent(id, wxCURL_DOWNLOAD_EVENT, originator, szURL),
      m_rDownloadTotal(rDownloadTotal),
      m_rDownloadNow(rDownloadNow) {}
...
wxCurlUploadEvent::wxCurlUploadEvent(
    int id, wxCurlBase* originator, const double& rUploadTotal,
    const double& rUploadNow, const std::string& szURL /*= wxEmptyString*/)
    : wxCurlProgressBaseEvent(id, wxCURL_UPLOAD_EVENT, originator, szURL),
      m_rUploadTotal(rUploadTotal),
      m_rUploadNow(rUploadNow) {}
...
wxCurlBeginPerformEvent::wxCurlBeginPerformEvent(int id,
                                                 const std::string& szURL)
    : wxEvent(id, wxCURL_BEGIN_PERFORM_EVENT), m_szURL(szURL) {}

wxCurlBeginPerformEvent::wxCurlBeginPerformEvent(
    const wxCurlBeginPerformEvent& event)
    : wxEvent(event) {
...
wxCurlEndPerformEvent::wxCurlEndPerformEvent(int id, const std::string& szURL,
...
bool wxCurlBase::Perform() {
  ...
  if ((m_nFlags & wxCURL_SEND_BEGINEND_EVENTS) && m_pEvtHandler) {
    wxString s = wxCURL_BUF2STRING(m_szCurrFullURL);
    wxCurlEndPerformEvent endEvent(m_nId, std::string(s.mb_str()),
                                   m_iResponseCode);
...
wxEvtHandler* wxCurlBase::GetEvtHandler() const { return m_pEvtHandler; }

int wxCurlBase::GetId() const { return m_nId; }

void wxCurlBase::SetFlags(long flags) { m_nFlags = flags; }

long wxCurlBase::GetFlags() const { return m_nFlags; }
...
void wxCurlBase::SetPort(const long& iPort) { m_iHostPort = iPort; }

long wxCurlBase::GetPort() const { return m_iHostPort; }
...
long wxCurlBase::GetResponseCode() const { return m_iResponseCode; }
...
void wxCurlBase::UseProxy(const bool& bUseProxy) { m_bUseProxy = bUseProxy; }

bool wxCurlBase::UseProxy() const { return m_bUseProxy; }
...
long wxCurlBase::GetProxyPort() const { return m_iProxyPort; }

void wxCurlBase::SetVerbose(const bool& bVerbose) { m_bVerbose = bVerbose; }

bool wxCurlBase::IsVerbose() const { return m_bVerbose; }
...
void wxCurlBase::SetAbort(bool a) { m_bAbortHungTransfer = a; }

bool wxCurlBase::GetAbort() const { return m_bAbortHungTransfer; }
//////////////////////////////////////////////////////////////////////
// Helper Methods
//////////////////////////////////////////////////////////////////////
...
void wxCurlBase::SetCurlHandleToDefaults(const wxString& relativeURL) {
  ...
  if (m_pCURL && ResetHandle()) {
...
#ifdef __WXMSW__
    SetOpt(CURLOPT_CAINFO,
           "curl-ca-bundle.crt");  // Use our local certificate list on Windows
    SetOpt(CURLOPT_SSL_VERIFYPEER,
           true);  // FIXME: Temporary until we get certificates working
#endif
    SetOpt(CURLOPT_ENCODING,
           "gzip,deflate");  // Save bandwidth by using compression
    ...
    if (!wxCURL_BUF_ISEMPTY(m_szUsername) ||
        !wxCURL_BUF_ISEMPTY(m_szPassword)) {
      wxString str = wxCURL_BUF2STRING(m_szUsername) + wxS(":") +
                     wxCURL_BUF2STRING(m_szPassword);
    ...
    if (m_bUseProxy && (!wxCURL_BUF_ISEMPTY(m_szProxyUsername) ||
                        !wxCURL_BUF_ISEMPTY(m_szProxyPassword))) {
      wxString str = wxCURL_BUF2STRING(m_szProxyUsername) + wxS(":") +
                     wxCURL_BUF2STRING(m_szProxyPassword);
...
void wxCurlBase::SetHeaders() {
  if (!m_arrHeaders.IsEmpty()) {
    ...
    for (unsigned int i = 0; i < m_arrHeaders.Count(); i++) {
      m_pHeaders =
          curl_slist_append(m_pHeaders, wxCURL_STRING2BUF(m_arrHeaders[i]));
    }
...
std::string wxCurlBase::GetCURLVersion() { return std::string(curl_version()); }
