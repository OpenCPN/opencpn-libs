...
bool wxCurlConnectionSettingsPanel::Create(wxWindow *parent, wxWindowID id,
  ...
  if (GetMinSize().GetWidth() < 250) SetMinSize(wxSize(250, -1));
...
wxTextCtrl *wxCurlConnectionSettingsPanel::AddSizerRow(wxSizer *sz,
                                                       const wxString &name,
                                                       bool grow) {
  // the static text
...
void wxCurlConnectionSettingsPanel::CreateControls(const wxString &msg) {
  ...
  wxFlexGridSizer *gs =
      new wxFlexGridSizer(0 /* calc automatically the # of rows */, 2, 0, 0);
  ...
  if (HasFlag(wxCCSP_PROXY_OPTIONS)) {
    wxSizer *proxy =
        new wxStaticBoxSizer(wxVERTICAL, this, _("Proxy settings"));
