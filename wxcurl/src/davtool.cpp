...
wxCurlDAVFs::wxCurlDAVFs() : m_iContentLength(0) {}

wxCurlDAVFs::wxCurlDAVFs(const wxString& szHREF, const wxString& szStatus,
...
bool wxCurlDAVTool::GetDAVFs(wxArrayDAVFs& fs,
      ...
      if (xmlPropfind.IsOk()) {
        ...
        while (pNode) {
          ...
          if (pNode->GetName().Find(wxS("response")) != -1) {
            if (ParseResponseXml(fsItem, pNode)) fs.Add(fsItem);
          }
...
bool wxCurlDAVTool::IsDirectory(
    const wxString& szRemoteLoc /*= wxEmptyString*/) {
  wxArrayDAVFs arrFs;
...
wxDateTime wxCurlDAVTool::GetLastModified(
    const wxString& szRemoteLoc /*= wxEmptyString*/) {
  wxArrayDAVFs arrFs;
...
wxDateTime wxCurlDAVTool::GetCreationDate(
    const wxString& szRemoteLoc /*= wxEmptyString*/) {
  wxArrayDAVFs arrFs;
...
long wxCurlDAVTool::GetContentLength(
    const wxString& szRemoteLoc /*= wxEmptyString*/) {
  wxArrayDAVFs arrFs;
...
wxString wxCurlDAVTool::GetContentType(
    const wxString& szRemoteLoc /*= wxEmptyString*/) {
  wxArrayDAVFs arrFs;
      ...
      if (pChild->GetName().Find(wxS("creationdate")) != -1) {
        ...
        while (pText) {
          if (pText->GetType() == wxXML_TEXT_NODE) {
            ...
            fsItem.m_dtCreationDate.ParseFormat(szVal,
                                                wxS("%Y-%m-%dT%H:%M:%S"));
