...
wxCurlHTTP::~wxCurlHTTP() { ResetPostData(); }
...
bool wxCurlHTTP::UseCookies() const { return m_bUseCookies; }
...
bool wxCurlHTTP::AddBufferToForm(const bool& bClear, const wxString& szName,
  ...
  res = curl_formadd(&m_pPostHead, &m_pPostTail, CURLFORM_COPYNAME,
                     (const char*)szName.ToAscii(), CURLFORM_COPYCONTENTS,
                     buffer, CURLFORM_CONTENTSLENGTH, len, CURLFORM_END);
...
bool wxCurlHTTP::Post(const char* buffer, size_t size,
...
bool wxCurlHTTP::Post(wxInputStream& buffer,
                      const wxString& szRemoteFile /*= wxEmptyString*/) {
  curl_off_t iSize = 0;
  ...
  if (m_pCURL && buffer.IsOk()) {
    ...
    if (iSize == (~(ssize_t)0))  // wxCurlHTTP does not know how to upload
...
bool wxCurlHTTP::Get(const wxString& szFilePath,
                     const wxString& szRemoteFile /*= wxEmptyString*/) {
...
size_t wxCurlHTTP::Get(char*& buffer,
                       const wxString& szRemoteFile /*= wxEmptyString*/) {
...
bool wxCurlHTTP::Get(wxOutputStream& buffer,
                     const wxString& szRemoteFile /*=wxEmptyString*/) {
  if (m_pCURL && buffer.IsOk()) {
    SetCurlHandleToDefaults(szRemoteFile);
...
bool wxCurlHTTP::Put(const wxString& szFilePath,
                     const wxString& szRemoteFile /*= wxEmptyString*/) {
...
bool wxCurlHTTP::Put(const char* buffer, size_t size,
                     const wxString& szRemoteFile /*= wxEmptyString*/) {
  wxMemoryInputStream inStream(buffer, size);
...
bool wxCurlHTTP::Put(wxInputStream& buffer,
                     const wxString& szRemoteFile /*= wxEmptyString*/) {
  curl_off_t iSize = 0;
