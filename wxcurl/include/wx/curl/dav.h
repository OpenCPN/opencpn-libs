...
public:
  ...
  // PROPFIND - Queries the Properties of the Resource Specified.  An XML Buffer
  // Can be Specified for Detailed Data.
  bool Propfind(const wxString& szRemoteLoc = wxEmptyString);
  bool Propfind(const wxArrayString& arrTags,
                const wxString& szRemoteLoc = wxEmptyString);
  bool Propfind(const char* buffer, size_t size,
                const wxString& szRemoteLoc = wxEmptyString);
  bool Propfind(wxInputStream& buffer,
                const wxString& szRemoteLoc = wxEmptyString);

  // PROPATCH - Alters the Properties of the Resource Specified.  An XML Buffer
  // Must be Specified to Modify Data.
  bool Proppatch(const char* buffer, size_t size,
                 const wxString& szRemoteLoc = wxEmptyString);
  bool Proppatch(wxInputStream& buffer,
                 const wxString& szRemoteLoc = wxEmptyString);

  // COPY - Copies the Specified Resource from one Location to Another.
  bool Copy(const wxString& szRemoteLocDest, const bool& bOverwrite = true,
            const wxString& szRemoteLocSrc = wxEmptyString);

  // MOVE - Moves the Specified Resource from one Location to Another.
  bool Move(const wxString& szRemoteLocDest, const bool& bOverwrite = true,
            const wxString& szRemoteLocSrc = wxEmptyString);
