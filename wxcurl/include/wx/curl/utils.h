...
//! This event gets posted by wxCurlSizeQueryThread. Use the
//! EVT_CURL_SIZE_QUERY(id, func) macro to intercept it.
...
public:
  ...
  wxEvent *Clone() const { return new wxCurlSizeEvent(*this); }
};
...
#define wxCurlSizeEventHandler(func)       \
  (wxObjectEventFunction)(wxEventFunction) \
      wxStaticCastEvent(wxCurlSizeEventFunction, &func)
...
//! This thread just retrieve the sizes of the given list of files and post them
//! to the registered event handler. This is useful to show the size of one or
//! more files to the user before downloading them.
...
public:
  ...
  wxCurlSizeQueryThread(wxEvtHandler *handler, int id,
  ...
  void SetURLs(const wxArrayString &urls) {
    wxCHECK_RET(!IsAlive(),
                wxS("Call this function before creating the thread!"));
    m_urls = urls;
  }
