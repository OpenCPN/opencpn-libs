...
public:
  wxCurlDAVFs();
  wxCurlDAVFs(const wxString& szHREF, const wxString& szStatus,
              const wxString& szCreationDate, const wxString& szLastModified,
              const long& iContentLength, const wxString& szContentType);
  ~wxCurlDAVFs();

  bool IsDirectory() const {
    return m_szContentType == wxS("httpd/unix-directory");
  }
...
public:
  ...
  bool HasDirectory(const wxString& szRemoteLoc = wxEmptyString) {
    return IsDirectory(szRemoteLoc);
  }
