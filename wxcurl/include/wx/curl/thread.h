...
enum wxCurlThreadError {
  ...
  wxCTE_NO_RESOURCE =
      wxTHREAD_NO_RESOURCE,  //!< There were insufficient resources to create a
                             //!< new thread.
  ...
  wxCTE_NO_VALID_STREAM,   //!< The input/output stream could not be created or
                           //!< is invalid.
  wxCTE_ABORTED,           //!< The thread was user-aborted through
                           //!< wxCurlBaseThread::Abort().
  wxCTE_CURL_ERROR
  //!< LibCURL failed. You can use thread->GetCurlSession()->GetErrorString() to
  //!< get more info.
};
...
protected:
  ...
  //! This flag is set to true when the thread has been cancelled using
  //! Delete(). Since it's continuosly read by the running thread and maybe
  //! written from other threads, it needs a mutex.
  bool m_bAbort;
...
public:
  ...
  ~wxCurlBaseThread() { m_pCurl = 0; }

public:  // thread execution management
  //! Returns true if this thread is ready to be started using e.g.
  //! #StartTransfer.
  virtual bool IsOk() const { return !m_url.empty() && m_pCurl != 0; }

  //! Starts the transfer. This is equivalent to call
  //! wxCurlDownloadThread::Download or wxCurlUploadThread::Upload.
...
public:  // setters
  //! Sets the event handler to which wxCurlBeginPerformEvent,
  //! wxCurlEndPerformEvent and wxCurlDownloadEvent/wxCurlUploadEvents will be
  //! posted.
  void SetEvtHandler(wxEvtHandler *handler, int id = wxID_ANY) {
    wxCHECK_RET(!IsAlive(),
                wxS("Cannot use this function after the tranfer has begun"));
    m_pHandler = handler;
    m_nId = id;
  }
  ...
  wxCurlThreadError SetURL(const wxString &url,
                           std::shared_ptr<wxCurlBase> pCurl);

public:  // getters
  wxEvtHandler *GetEvtHandler() const { return m_pHandler; }
  int GetId() const { return m_nId; }

  bool IsAborting() { return m_bAbort; }

  //! Returns the wxCurlBase-derived object which is being used for the
  //! transfer. Note that the returned value will be NULL if you've not called
  //! #SetURL yet. You can cast it to the wxCurlBase-derived class associated
  //! with the return value of GetProtocol() (e.g. if GetProtocol() returns
  //! wxCP_HTTP, you can cast GetCurlSession() to wxCurlHTTP).
  wxCurlBase *GetCurlSession() const { return m_pCurl.get(); }

  std::shared_ptr<wxCurlBase> &GetCurlSharedPtr() { return m_pCurl; }
  ...
  wxCurlProtocol GetProtocol() const { return m_protocol; }
...
protected:
  ...
  virtual wxCurlThreadError Run() { return (wxCurlThreadError)wxThread::Run(); }
...
public:
  ...
  void SetStream(wxOutputStream *realStream) { m_stream = realStream; }

  virtual size_t OnSysWrite(const void *buffer, size_t bufsize);

  virtual bool IsOk() const { return m_thread && m_stream && m_stream->IsOk(); }

  wxOutputStream *GetRealStream() const { return m_stream; }

  wxFileOffset GetLength() const { return m_stream->GetLength(); }
...
class WXDLLIMPEXP_CURL wxCurlDownloadThread : public wxCurlBaseThread {
  friend class wxCurlDownloadThreadOutputFilter;  // needs to access our
                                                  // TestDestroy()
...
public:
  wxCurlDownloadThread(wxEvtHandler *handler = NULL, int id = wxID_ANY,
                       const wxString &url = wxEmptyString,
                       wxOutputStream *out = NULL)
      : wxCurlBaseThread(handler, id), m_output(this) {
    if (!url.IsEmpty()) Download(url, out);
  }
...
public:  // public API
  ...
  wxOutputStream *GetOutputStream() const {
    wxCHECK_MSG(!IsRunning(), NULL,
                wxS("You cannot access the output stream while the thread is "
                    "running!"));
...
protected:
  // change access policy to force the user of the better-readable Download()
  // method:
  virtual wxCurlThreadError StartTransfer() { return Download(); }
...
public:
  ...
  void SetStream(wxInputStream *realStream) { m_stream = realStream; }

  virtual size_t OnSysRead(void *buffer, size_t bufsize);

  virtual bool IsOk() const { return m_thread && m_stream && m_stream->IsOk(); }

  wxInputStream *GetRealStream() const { return m_stream; }

  wxFileOffset GetLength() const { return m_stream->GetLength(); }

  char Peek() { return m_stream->Peek(); }
};
...
class WXDLLIMPEXP_CURL wxCurlUploadThread : public wxCurlBaseThread {
  friend class wxCurlUploadThreadInputFilter;  // needs to access our
                                               // TestDestroy()
...
public:
  wxCurlUploadThread(wxEvtHandler *handler = NULL, int id = wxID_ANY,
                     const wxString &url = wxEmptyString,
                     wxInputStream *in = NULL)
      : wxCurlBaseThread(handler, id), m_input(this) {
    if (!url.IsEmpty()) Upload(url, in);
  }
...
protected:
  // change access policy to force the user of the better-readable Upload()
  // method:
  virtual wxCurlThreadError StartTransfer() { return Upload(); }
