    ...
    for (auto &s : subschemata_) {
      ...
      if (err) {
        ...
        if (combine_logic == allOf) {
          e.error("", instance,
                  "at least one schema has failed, but ALLOF them are required "
                  "to validate.");
          return;
        }
      } else
      ...
      if (combine_logic == oneOf && count > 1) {
        e.error("", instance,
                "more than one schema has succeeded, but only ONEOF them is "
                "required to validate.");
        return;
      }
      if (combine_logic == anyOf && count == 1) return;
    }

    if ((combine_logic == anyOf || combine_logic == oneOf) && count == 0)
      e.error("", instance,
              "no validation has succeeded but ANYOF/ONEOF them is required to "
              "validate.");
  }
...
class type_schema : public schema {
  ...
  void validate(const json &instance,
                basic_error_handler &e) const override final {
    ...
    for (auto l : logic_) l->validate(instance, e);
    ...
    if (if_) {
      ...
      if (!err) {
        if (then_) then_->validate(instance, e);
      } else {
        if (else_) else_->validate(instance, e);
      }
    }
  }
...
public:
  type_schema(json &sch, root_schema *root,
    ...
    // with nlohmann::json floats can be seen as unsigned or integer - reuse the
    // number-validator for integer values as well, if they have not been
    // specified
  ...
  void validate(const json &instance, basic_error_handler &e) const override {
    if (minLength_.first) {
      if (utf8_length(instance) < minLength_.second) {
        std::ostringstream s;
        s << "'" << instance << "' is too short as per minLength ("
          << minLength_.second << ")";
    ...
    if (maxLength_.first) {
      if (utf8_length(instance) > maxLength_.second) {
        std::ostringstream s;
        s << "'" << instance << "' is too long as per maxLength ("
          << maxLength_.second << ")";
        e.error("", instance, s.str());
      }
    }
    ...
    if (pattern_.first && !REGEX_NAMESPACE::regex_search(
      ...
      e.error("", instance,
              instance.get<std::string>() +
                  " does not match regex pattern: " + patternString_);
#endif

    if (format_.first) {
      if (format_check_ == nullptr)
        e.error("", instance,
                std::string("A format checker was not provided but a "
                            "format-attribute for this string is present. ") +
...
class numeric : public schema {
  ...
  bool violates_multiple_of(json::number_float_t x) const {
    json::number_integer_t n =
        static_cast<json::number_integer_t>(x / multipleOf_.second);
  ...
  void validate(const json &instance, basic_error_handler &e) const override {
    ...
    if (multipleOf_.first && value != 0)  // zero is multiple of everything
      if (violates_multiple_of(value))
        e.error("", instance,
                "is not a multiple of " + std::to_string(multipleOf_.second));

    if (maximum_.first)
      if ((exclusiveMaximum_ && value >= maximum_.second) ||
          value > maximum_.second)
        e.error("", instance,
                "exceeds maximum of " + std::to_string(maximum_.second));

    if (minimum_.first)
      if ((exclusiveMinimum_ && value <= minimum_.second) ||
          value < minimum_.second)
        e.error("", instance,
                "is below minimum of " + std::to_string(minimum_.second));
  }
...
class null : public schema {
  void validate(const json &instance, basic_error_handler &e) const override {
    if (!instance.is_null()) e.error("", instance, "expected to be null");
  }
...
class boolean : public schema {
  ...
  void validate(const json &instance, basic_error_handler &e) const override {
    if (!true_) {  // false schema
      ...
      //		e.error("", instance, "false-schema required empty
      //array"); 	return;
      //}
...
class object : public schema {
  ...
  void validate(const json &instance, basic_error_handler &e) const override {
    ...
    for (auto &p : instance.items()) {
      if (propertyNames_) propertyNames_->validate(p.key(), e);
...
class array : public schema {
  ...
  void validate(const json &instance, basic_error_handler &e) const override {
    ...
    if (items_schema_)
      for (auto &i : instance) items_schema_->validate(i, e);
    else {
...
class root_schema : public schema {
  ...
  struct schema_file {
    ...
    std::map<nlohmann::json::json_pointer, std::shared_ptr<schema_ref>>
...
public:
  root_schema(
      std::function<void(const json_uri &, json &)> loader,
  ...
  void insert(const json_uri &uri, const std::shared_ptr<schema> &s) {
    ...
    // std::cout << "resolving schemas looking for '" << uri.pointer() << "' in
    // " << uri.location() << "\n";
...
std::shared_ptr<schema> schema::make(json &schema, root_schema *root,
  ...
  for (auto &key : keys)
    for (auto &uri : uris) uri = uri.append(json_uri::escape(key));
...
class throwing_error_handler : public basic_error_handler {
  void error(const std::string &path, const json &instance,
             const std::string &message) {
    throw std::invalid_argument(std::string("At ") + path + " of " +
