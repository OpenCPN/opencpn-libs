...
void json_uri::update(const std::string &uri) {
  ...
  if (pointer_separator !=
      std::string::npos) {  // and extract the pointer-string if found
  ...
  if (location.size()) {  // a location part has been found
    pointer_ =
        ""_json_pointer;  // if a location is given, the pointer is emptied
    ...
    if (location.find("urn:") == 0) {
      ...
      if (proto != std::string::npos) {  // extract the protocol
        ...
        if (hostname != std::string::npos) {  // and the hostname (no proto
                                              // without hostname)
      ...
      if (urn_.size() && path.size())
        throw std::invalid_argument("Cannot add a path (" + path +
                                    ") to an URN URI (" + urn_ + ")");

      if (path[0] == '/')  // if it starts with a / it is root-path
        path_ = path;
      else if (pos ==
               0) {  // the URL contained only a path and the current path has
                     // no / at the end, strip last element until / and append
...
const std::string json_uri::location() const {
  if (urn_.size()) return urn_;

  std::stringstream s;

  if (proto_.size() > 0) s << proto_ << "://";
...
std::string json_uri::escape(const std::string &src) {
  std::vector<std::pair<std::string, std::string>> chars = {{"~", "~0"},
                                                            {"/", "~1"}};
